...
| `BACKEND_CORS_ORIGINS` | string[] | CORS許可オリジン（カンマ区切り）。パターンAでは `*` を推奨し、CORSは全オリジン許可とした上でテナントごとの`allowed_widget_origins`でOrigin制御を行います。 | `"*"` | `"*"` |
...
| 変数名 | 型 | 説明 | デフォルト値 | 例 |
|---|---|---|---|---|
| `WIDGET_CDN_URL` | string | 埋め込みコード生成時に使用するウィジェットCDN URL（本番環境では推奨） | `None` | `https://widget.example.com/widget.js` |
| `API_BASE_URL` | string | 埋め込みコード生成時に使用するAPIベースURL（本番環境では推奨） | `None` | `https://api.example.com/api/v1` |

**注意**: 
- `WIDGET_CDN_URL` が未設定の場合、埋め込みコード生成時にデフォルト値 `https://cdn.rag-chatbot.com/widget.js` が使用されます。
- `API_BASE_URL` が未設定の場合、`APP_URL` から自動構築されます（`APP_URL/api/v1`）。どちらも未設定の場合は相対パス `/api/v1` が使用されます。
- 本番環境では適切な値を設定することを強く推奨します。
...
| `NEXT_PUBLIC_API_BASE_URL` | string | APIベースURL（ブラウザ側でアクセス可能、フロントエンドのAPIクライアント用） | `http://localhost:8000` | `https://api.example.com` |
| `NEXT_PUBLIC_API_URL` | string | API URL（ブラウザ側でアクセス可能、埋め込みコード生成用） | `None` | `https://api.example.com/api/v1` |
