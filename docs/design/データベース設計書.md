...
### 3.9 tenants（テナント）拡張: allowed_widget_origins

| カラム名 | データ型 | 制約 | 説明 |
|---|---|---|---|
| allowed_widget_origins | TEXT | NULL | ウィジェット設置を許可するオリジンのCSV文字列（例: `https://example.com,https://sub.example.com`）。テナントごとに1〜3件程度を想定。アプリケーション層で分割してリストとして扱い、`Origin` ヘッダと突き合わせてウィジェットアクセスを制御します。 |
