...
1. ヘッダーメニューから「テナント設定」をクリック（またはダッシュボードから「テナント設定を開く」ボタンをクリック）
2. 設定画面が表示されます
...
### 7. ウィジェット埋め込み（外部サイト連携）

自社WebサイトやWebアプリケーションにチャットボットを埋め込むための機能です。

#### 前提条件

- テナント管理者がチャット用モデル・ベクトル埋め込みモデル・APIキーを設定済みであること
- （推奨）システム管理者がウィジェット用のCDN URL・APIベースURLなどの環境変数を設定済みであること

#### 埋め込みコード（スニペット）の取得

1. テナント管理者としてログイン
2. 設定または専用メニューから「ウィジェット」または「埋め込みコード」画面を開く
3. テナントごとのウィジェット用APIキーやテナントIDが自動で反映された埋め込みコードが表示されます
4. 表示されている `<script>` タグまたはコードブロック全体をコピーします

#### Webサイトへの埋め込み

1. 自社WebサイトのHTMLファイル、もしくはCMSのカスタムHTMLエリアを開きます
2. チャットボットを表示したいページの `<body>` 終了タグ直前などに、コピーした埋め込みコードを貼り付けます
3. ページを保存・公開します
4. 対象ページをブラウザで開き、画面右下などにチャットボットのウィジェットが表示されることを確認します

#### 注意事項

- ウィジェット用APIキーやテナントIDは、第三者に共有しないよう注意してください
- ステージング環境と本番環境でウィジェットCDNやAPIベースURLが異なる場合があります。環境に応じたコードを利用してください
- ウィジェットが表示されない場合は、ブラウザの開発者ツールでコンソールエラーやネットワークエラーを確認してください
...
### フロー4：Webサイトにチャットボットを埋め込む

1. **ウィジェット設定を確認（テナント管理者）**
   - テナント設定やウィジェット設定画面から、ウィジェット用APIキーやテナントIDが発行されていることを確認
2. **埋め込みコードを取得**
   - 管理画面の「ウィジェット」または「埋め込みコード」画面から、表示されているスニペット（`<script>` タグなど）をコピー
3. **Webサイトに貼り付け**
   - 自社WebサイトのHTML（またはCMSのカスタムHTML）にコードを貼り付け、保存・公開
4. **表示確認**
   - 対象ページを開き、チャットボットのウィジェットが表示されるか確認
5. **動作確認**
   - 埋め込んだウィジェットから質問を行い、想定どおり回答されるか確認
...
- **ウィジェット**：外部サイトに埋め込むチャットボット用のUIコンポーネント
- **ウィジェットCDN**：ウィジェット用JavaScriptファイルなどを配信するコンテンツ配信ネットワーク
