...
| 変数名 | 型 | 説明 | デフォルト値 | 例 |
|---|---|---|---|---|
| `DATABASE_URL` | string | PostgreSQL接続URL（pgvector拡張有） | `postgresql+asyncpg://user:password@db:5432/ai_chatbot_db` | `postgresql+asyncpg://user:pass@localhost:5432/db` |
...
| 変数名 | 型 | 説明 | デフォルト値 | 例 |
|---|---|---|---|---|
| `STORAGE_LOCAL_PATH` | string | ローカルストレージパス | `/app/uploads` | `./storage/uploads` |
...
| `NEXT_PUBLIC_WIDGET_URL` | string | ウィジェットCDN URL（ブラウザ側でアクセス可能） | `http://localhost:3001/widget.js` | `https://cdn.example.com/widget.js` |
...
```bash
# データベース
DATABASE_URL=postgresql+asyncpg://user:password@staging-db:5432/ai_chatbot_db
...
```bash
# データベース
DATABASE_URL=postgresql+asyncpg://user:password@prod-db:5432/ai_chatbot_db
