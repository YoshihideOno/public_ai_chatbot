...
- 本番環境でウィジェットを動作させるには、管理画面のテナント設定画面で「設置ドメイン」を登録する必要があります。
- テナント設定 → 「テナント設定 - コンテンツ処理完了通知」カード内の「設置ドメイン」に、静的サイトのオリジン（例: `https://ai-chatbot-project-staticsite.vercel.app`）をカンマ区切りで入力してください。
- バックエンドは、この設置ドメイン（`allowed_widget_origins`）とリクエストの `Origin` ヘッダを比較し、一致するドメインからのウィジェットチャットのみ受け付けます。
