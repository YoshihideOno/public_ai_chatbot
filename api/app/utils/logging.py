...
class StructuredLogger:
    ...
    def __init__(self, name: str = "rag_chatbot"):
        ...
        # 親ロガーへの伝播を無効化（ルートロガーのハンドラーと重複しないように）
...
class BusinessLogger:
    ...
    def log_user_action(
        ...
        **kwargs
        ...
        このメソッドは「重要な変更系操作」および
        一部のセキュリティ／システムイベントを対象として、
        構造化ログと監査ログ（audit_logs）の両方に記録する。
        
        注意:
            - ダッシュボード表示や一覧取得などの参照系GETは、
              audit_logsの肥大化を防ぐため原則として対象外とする。
            - 参照系であっても、エクスポートや機微情報アクセスなど
              監査が必要な操作のみ、明示的に本メソッドを呼び出す。
